...
function bind(el, { value }) {
  const { events, handler, middleware, isActive, detectIframe } = processDirectiveArguments(value)
  if (!isActive) {
    return
  }
...
function unbind(el) {
  ...
  handlers.forEach(({ event, srcTarget, handler }) => srcTarget.removeEventListener(event, handler, false))
